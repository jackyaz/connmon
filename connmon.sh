...
Update_Check(){
	...
	if [ "$localver" != "$serverver" ]; then
		...
		printf "\"Changelog\\n%s\"" "$changelog" > "$SCRIPT_WEB_DIR/detect_changelog.htm"
	else
...
Conf_Exists(){
	if [ -f "$SCRIPT_CONF" ]; then
		...
		echo "NOTIFICATIONS_INFLUXDB_HOST="; echo "NOTIFICATIONS_INFLUXDB_PORT=8086"; echo "NOTIFICATIONS_INFLUXDB_DB=connmon"; echo "NOTIFICATIONS_INFLUXDB_VERSION=1.8"; echo "NOTIFICATIONS_INFLUXDB_USERNAME="; echo "NOTIFICATIONS_INFLUXDB_PASSWORD="; echo "NOTIFICATIONS_INFLUXDB_APITOKEN="; } > "$SCRIPT_CONF"
		return 1
	fi
}
